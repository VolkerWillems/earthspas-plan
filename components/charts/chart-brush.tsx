...
const ChartBrushInner = memo(function ChartBrushInner({
  ...
  const initialBrushPosition = useMemo(() => {
    if (!initialSelection || innerWidth <= 0 || innerHeight <= 0) return undefined;
    const xScaleFn = xScale as (d: Date) => number;
  ...
  const updatePixelExtent = useCallback(
    (bounds: Bounds | null) => {
      ...
      if (pixels) setPixelExtent(pixels);
    },
    [innerWidth, xScale]
  );
...
export function ChartBrush({
  ...
  const notifySelectionChange = useCallback(
    (bounds: Bounds | null) => {
      if (onSelectionChange) onSelectionChange(boundsToSelection(bounds));
    },
    [onSelectionChange, boundsToSelection]
  );

  if (!isLoaded || innerWidth <= 0 || innerHeight <= 0) return null;
