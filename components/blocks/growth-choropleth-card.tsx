...
import { ArrowUpRight, Globe, Target } from "@/lib/phosphor-icons";
...
const markets = [
  {
    ...
    share: 45,
    path: "M87 89l22-12 20 8 7 19-8 22-20 6-18-13-8-18z",
    labelX: 108,
    labelY: 106,
  },
  {
    id: "de",
    label: "Duitsland · NRW",
    status: "Groeiprioriteit",
    share: 35,
    path: "M140 74l47-11 34 18 9 37-18 41-39 12-31-22-13-37z",
    labelX: 181,
    labelY: 114,
  },
  {
    ...
    status: "Tweede fase",
    share: 12,
    path: "M66 126l39-5 20 18-10 23-35 2-20-16z",
    labelX: 91,
    labelY: 146,
  },
  {
    id: "lu",
    label: "Luxemburg",
    status: "Compacte premiumtest",
    share: 8,
    path: "M122 153l15-3 10 12-7 16-15-2-7-11z",
    labelX: 133,
    labelY: 166,
  },
] as const;
          ...
          <div className="growth-map-copy">
            <div className="growth-map-heading">
              ...
              <div>
                ...
                <h1 id="growth-map-title">Van sterke basis naar meetbare regionale groei</h1>
              </div>
            </div>

            <p className="growth-map-intro">
              De kaart vertaalt het huidige werkbudget naar een voorzichtig groeiscenario. Nederland en NRW staan voorop; België en Luxemburg volgen pas wanneer tracking, opvolging en conversie aantoonbaar werken.
            </p>

            <div className="growth-map-metrics" aria-label="Berekend groeiscenario">
              <div>
                <span>Werkbasis</span>
                <strong>+{number.format(model.baseExtraSales)} spa&apos;s</strong>
                <small>berekende extra verkopen per jaar</small>
              </div>
              <div>
                <span>Extra omzet</span>
                <strong>{euro.format(model.baseExtraRevenue)}</strong>
                <small>scenario, geen omzetgarantie</small>
              </div>
              <div>
                <span>Omzetgroei</span>
                <strong>+{number.format(model.growthPct)}%</strong>
                <small>op basis van de huidige invoer</small>
              </div>
            </div>

            <div className="growth-map-market-detail" aria-live="polite">
              <div>
                <span>{activeMarket.status}</span>
                <h2>{activeMarket.label}</h2>
              </div>
              <div className="growth-map-market-result">
                <ArrowUpRight aria-hidden="true" />
                <p><strong>circa {number.format(marketSales)} verkopen</strong><span>{euro.format(marketRevenue)} extra omzet in de werkbasis</span></p>
              </div>
            </div>
            ...
            <svg viewBox="0 0 300 230" role="img" aria-labelledby="growth-map-svg-title">
              ...
              {markets.map((market, index) => (
                <g
                  ...
                  onKeyDown={(event) => {
                    if (event.key === "Enter" || event.key === " ") setActiveId(market.id);
                  }}
                >
            ...
            <div className="growth-map-legend">
              {markets.map((market) => (
                <button
                  key={market.id}
                  type="button"
                  className={activeId === market.id ? "is-active" : undefined}
                  onClick={() => setActiveId(market.id)}
                >
                  <span>{market.share}%</span>
                  <span><strong>{market.label}</strong><small>{market.status}</small></span>
                </button>
              ))}
            </div>
          </div>
